...
def daily_min(data):
    ...
    :param data: A 2D data array with inflammation data (each row contains measurements for a single
        patient across all days).
    :return: An array of min values of measurements for each day.
    """
...
def patient_normalise(data):
    ...
    if not isinstance(data, np.ndarray):
        raise TypeError('Inflammation values should be provided as NumPy array')
    if data.ndim != 2:
        raise ValueError('Inflammation data array should be 2-D')
    if np.any(data < 0):
